...
public class ShouldSourceText {
     ...
     */
    public ShouldSourceText(ExpressionStatement expr, SourceUnit sourceUnit, Janitor janitor) {
        if (!hasPlausibleSourcePosition(expr))
            throw new ShouldSourceNotAvailableException(expr, sourceUnit, "Invalid source position");
        ...
        for (int line = expr.getLineNumber(); line <= expr.getLastLineNumber(); line++) {
            ...
            if (lineText == null)
                throw new ShouldSourceNotAvailableException(expr, sourceUnit, "SourceUnit.getSample() returned null");
