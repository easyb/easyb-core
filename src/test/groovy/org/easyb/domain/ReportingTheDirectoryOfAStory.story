...
scenario "the story file should be stored in the Groovy binding",  {
  given "a story to be read from a directory", {
	  storyFilePath = "${testSourceDir}/groovy/org/easyb/bdd/story/stack/EmptyStackStory.groovy"
...
scenario "the specification file should be stored in the Groovy binding",  {
	given "a story to be read from a directory", {
      ...
      specFilePath = "${testSourceDir}${pathsep}groovy${pathsep}org${pathsep}easyb${pathsep}bdd${pathsep}specification${pathsep}queue${pathsep}QueueSpecification.groovy"
	  spec = new Specification(new GroovyShellConfiguration(), "a phrase", new File("${testSourceDir}/groovy/org/easyb/bdd/specification/queue/QueueSpecification.groovy"))
	}
